...
def sanitize_input(input_str):
    ...
    input_str = re.sub(r'<script\b[^>]*>(.*?)</script>', '', input_str, flags=re.IGNORECASE)

    # Remove tags that commonly lead to XSS if not needed (like iframe, object, embed, etc.)
    input_str = re.sub(r'</?(iframe|embed|object|form|input|button|style|link|meta)\b[^>]*>', '', input_str, flags=re.IGNORECASE)
    ...
    input_str = re.sub(r'\s(href|src)\s*=\s*("|\')?javascript:[^"\']*("|\')?', '', input_str, flags=re.IGNORECASE)
